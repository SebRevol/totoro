...
if __name__ == '__main__':
    ...
    shift_duration = duration /(len(all_players)+4)
    ...
    for index in range (size+4) :
        if (index < size):
            minus_1 = all_players[index]
        else :
            minus_1 = None
        if (index >0 and index < size+1):
            one = all_players[index-1]
        else :
            one = None
        if (index >1 and  index < size+2):
            center = all_players[index-2]
        else:
            center = None
        if (index >2 and index < size +3):
            two = all_players[index-3]
        else :
            two = None
            
        if (index >3):
            plus_1 = all_players[index-4]
        else :
            plus_1 =None
